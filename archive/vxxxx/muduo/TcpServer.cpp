...
TcpServer::TcpServer(
    EventLoop* loop, const InetAddress& listenAddr, const std::string& nameArg, Option option)
...
TcpServer::~TcpServer() {
    for (auto& item : connections_) {
        // 这个局部的 shared_ptr 智能指针对象,
        // 出右括号, 可以自动释放 new 出来的 TcpConnection 对象资源了
...
void TcpServer::newConnection(int sockfd, const InetAddress& peerAddr) {
    ...
    LOG_INFO(
        "TcpServer::newConnection [%s] - new connection [%s] from %s \n", name_.c_str(),
